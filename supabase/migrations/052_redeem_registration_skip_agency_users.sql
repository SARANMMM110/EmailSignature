-- Prevent a stored signup ?ref= from overwriting profiles.plan after the user joins an agency
-- (race: RegistrationRedeemBootstrap vs agency join could apply the ref second and bump tier).
  ...
  if exists (
    ...
    where id = p_user_id
      and (
        ...
        or coalesce(is_agency_owner, false) = true
      )
  ) then
    return jsonb_build_object(
      'ok', false,
      'error', 'SKIP_AGENCY_USER',
      'message', 'Plan is managed by your agency membership; signup ref was not applied.'
    );
  end if;
