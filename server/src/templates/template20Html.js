/**
 * Layout 20 — neon dark banner (email rail): radial field + hex mesh, swirl behind portrait,
 * condensed sans name, gradient role chip, portrait gutter, stroke icons, optional logo.
 * Shell width `sig_t20_outer_width`: **600px** with photo, **412px** without (188px center column omitted).
 * Field/deco SVGs are authored at 600px wide; CSS scales via `background-size` / img width when narrower.
 */
...
export const TEMPLATE_20_MARKUP = `<!-- T20 — neon dark; shell {{sig_t20_outer_width}}px × ${T20_H}px -->
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="{{sig_t20_outer_width}}" style="width:{{sig_t20_outer_width}}px;max-width:100%;border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt;font-family:${T20_FONT};">
<tr>
<td align="center" valign="top" style="padding:0;vertical-align:top;">
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="{{sig_t20_outer_width}}" style="width:{{sig_t20_outer_width}}px;max-width:100%;border-collapse:collapse;">
<tr>
<td width="{{sig_t20_outer_width}}" height="${T20_H}" valign="top" bgcolor="{{t20_bg}}" background="{{{t20_field_uri}}}" style="width:{{sig_t20_outer_width}}px;height:${T20_H}px;max-width:100%;padding:0;vertical-align:top;border:0;border-radius:2px;background-color:{{t20_bg}};background-image:url({{{t20_field_uri}}});background-repeat:no-repeat;background-position:50% 50%;background-size:{{sig_t20_outer_width}}px ${T20_H}px;mso-border-radius-alt:2px;overflow:hidden;position:relative;">
<img src="{{{t20_deco_uri}}}" width="{{sig_t20_outer_width}}" height="${T20_H}" alt="" style="display:block;width:{{sig_t20_outer_width}}px;height:${T20_H}px;border:0;position:absolute;left:0;top:0;z-index:0;line-height:0;font-size:0;opacity:0.72;">
...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="{{sig_t20_outer_width}}" height="${T20_H}" style="width:{{sig_t20_outer_width}}px;height:${T20_H}px;border-collapse:collapse;table-layout:fixed;position:relative;z-index:1;">
<tr>
...
{{/if}}
<td width="${T20_R}" valign="middle" height="${T20_H}" style="width:${T20_R}px;max-width:${T20_R}px;height:${T20_H}px;padding:10px {{#if sig_has_photo_column}}118px{{else}}14px{{/if}} 8px 6px;vertical-align:middle;background-color:transparent;overflow:hidden;box-sizing:border-box;">
